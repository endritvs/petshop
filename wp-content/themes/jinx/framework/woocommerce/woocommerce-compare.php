...
if(!class_exists('Jinx_WooCommerce_Compare')){
    class Jinx_WooCommerce_Compare {
        ...
        public function remove_item_from_url(){
            if(isset($_GET['la_helpers_compare_remove']) && ( $product_id = absint($_GET['la_helpers_compare_remove'])) ) {
                ...
                if(($key = array_search($product_id, $product_exists)) !== false) {
                    ...
                    wc_add_notice( sprintf( esc_html__('"%1$s" has been removed from list', 'jinx'), get_the_title($product_id) ) );
                }
            }
        }
        ...
        public function ajax_process(){
            ...
            if(apply_filters('jinx/check_ajax_referer', false, 'compare_nonce', 'security')){
            ...
            if( $check_ajax_referer ) {
                if(!isset($_REQUEST['post_id'])){
                    wp_send_json_error(array(
                        'message' => esc_html__('Invalid Product ID', 'jinx')
                    ));
                }
                ...
                wp_send_json_error(array(
                    'message' => esc_html__('Invalid security key', 'jinx')
                ));
            }
        }
        ...
        private function add( $post_id = 0 ){
            $lists = $this->get_data();
            $response = array();
            $response['compare_url'] = jinx_get_compare_url();
            if(in_array($post_id, $lists)){
                $response['message'] = esc_html__('Product already in list', 'jinx');
                $response['has_error'] = true;
            }
            else{
                $response['message'] = esc_html__('Product added', 'jinx');
        ...
        private function remove( $post_id = 0 ){
            ...
            if(($key = array_search($post_id, $lists)) !== false) {
                $response['message'] = esc_html__('Product has been removed from list', 'jinx');
                $response['has_error'] = false;
                unset($lists[$key]);
                $this->set_lists_for_user($lists);
                $this->set_lists_for_cookie($lists);
                ...
                $response['message'] = esc_html__('Product does not exist in list', 'jinx');
                $response['has_error'] = true;
            }
        ...
        public static function get_default_attributes(){
            return array(
                'image'         => esc_html__( 'Image', 'jinx' ),
                'title'         => esc_html__( 'Title', 'jinx' ),
                'add-to-cart'   => esc_html__( 'Add to cart', 'jinx' ),
                'price'         => esc_html__( 'Price', 'jinx' ),
                'sku'           => esc_html__( 'Sku', 'jinx' ),
                'description'   => esc_html__( 'Description', 'jinx' ),
                'stock'         => esc_html__( 'Availability', 'jinx' ),
                'weight'        => esc_html__( 'Weight', 'jinx' ),
                'dimensions'    => esc_html__( 'Dimensions', 'jinx' )
            );
        }

        public function output( $atts, $output ) {
            ob_start();
