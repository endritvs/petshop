<?php
/**
 * Jinx Breadcrumb Trail class
 *
 * @package Jinx WordPress theme
 * @since 1.0.0
 */

if ( !function_exists('jinx_breadcrumb_trail') ) {
    function jinx_breadcrumb_trail( $args = array() ) {

        // Return if breadcrumbs are disabled
        if ( ! apply_filters( 'jinx/filter/display_breadcrumbs', true ) || is_front_page() ) {
            return;
        }
        ...
        if ( !is_object( $breadcrumb ) )
            $breadcrumb = new Jinx_Breadcrumb_Trail( $args );
...
if( !function_exists('jinx_sp_breadcrumbs_before') ) {
    function jinx_sp_breadcrumbs_before() {
        $classes = 'site-breadcrumbs';
        echo '<div class="'. esc_attr($classes) .'">';
    }
}
add_action( 'seopress_breadcrumbs_before_html', 'jinx_sp_breadcrumbs_before' );

if(!function_exists('jinx_sp_breadcrumbs_after')){
    function jinx_sp_breadcrumbs_after() {
        echo '</div>';
    }
}
add_action( 'seopress_breadcrumbs_after_html', 'jinx_sp_breadcrumbs_after' );
...
if( !class_exists('Jinx_Breadcrumb_Trail') ) {

    class Jinx_Breadcrumb_Trail {
        ...
        /**
         * Sets up the breadcrumb trail properties.  Calls the `Jinx_Breadcrumb_Trail::add_items()` method
         ...
         *     @type array     $labels         Text labels. @see Jinx_Breadcrumb_Trail::set_labels()
         *     @type array     $post_taxonomy  Taxonomies to use for post types. @see Jinx_Breadcrumb_Trail::set_post_taxonomy()
        ...
        public function get_trail() {
            ...
            $breadcrumb    	= '';
            $separator      = apply_filters( 'breadcrumb_trail_separator', jinx_get_option('breadcrumb_separator', '>') );
            ...
            if ( 0 < $item_count ) {
                ...
                $breadcrumb .= '<ul class="trail-items"'. jinx_get_schema_markup( 'breadcrumb' ) .'>';

                // Add the number of items and item list order schema.
                if(is_paged() && jinx_get_option( 'schema_markup', false )){
                ...
                foreach ( $this->items as $item ) {
                    ...
                    $item = !empty( $matches ) ? sprintf( '%s<span'. jinx_get_schema_markup( 'author_name' ) .'>%s</span>%s', $matches[1], $matches[2], $matches[3] ) : sprintf( '<span'. jinx_get_schema_markup( 'author_name' ) .'>%s</span>', $item );
                    ...
                    $attributes = 'class="' . $item_class . '"'. jinx_get_schema_markup( 'breadcrumb_list' ) .'';

                    // Separator
                    if ( $item_count === $item_position ) {
                        $sep = '';
                        if(jinx_get_option( 'schema_markup', false )){
                            $current_url = home_url(add_query_arg(null, null));
                            $tmp = sprintf( '<meta content="%s"'. jinx_get_schema_markup( 'item' ) .' />', $current_url );
                            $item .= $tmp;
                        }
                    } else {
                        $sep = $separator;
                    ...
                    if(jinx_get_option( 'schema_markup', false )){
                        $meta = sprintf( '<meta content="%s"'. jinx_get_schema_markup( 'position' ) .' />', absint( $item_position ) );
                    }
                    else{
                        $meta = '';
                    }
                ...
                $breadcrumb = sprintf(
                    '<%1$s aria-label="%2$s" class="site-breadcrumbs'. $p_class .'"'. jinx_get_schema_markup( 'breadcrumb_itemprop' ) .'>%3$s%4$s%5$s</%1$s>',
        ...
        public function trail() {
            ...
            echo jinx_render_variable( $this->get_trail() );
        }
        ...
        protected function set_labels() {
            ...
            $defaults = array(
                'aria_label'          	=> esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'jinx' ),
                'home'           		=> jinx_get_option( 'breadcrumb_translation_home', esc_html__( 'Home', 'jinx' ) ),
                'error_404'           	=> jinx_get_option( 'breadcrumb_translation_error', esc_html__( '404 Not Found', 'jinx' ) ),
                'archives'            	=> esc_html__( 'Archives',                              'jinx' ),
                // Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
                'search'           		=> jinx_get_option( 'breadcrumb_translation_search', esc_html__( 'Search results for', 'jinx' ) ),
                // Translators: %s is the page number.
                'paged'               	=> esc_html__( 'Page %s',                               'jinx' ),
                // Translators: Minute archive title. %s is the minute time format.
                'archive_minute'      	=> esc_html__( 'Minute %s',                             'jinx' ),
                // Translators: Weekly archive title. %s is the week date format.
                'archive_week'        	=> esc_html__( 'Week %s',                               'jinx' ),
        ...
        protected function add_network_home_link() {
            ...
            // Home item
            $setting = jinx_get_option( 'breadcrumb_home_item', 'icon' );
            ...
            if ( is_multisite() && ! is_main_site() && true === $this->args['network'] )
                $this->items[] = sprintf( '<a href="%s" rel="home" aria-label="' .$this->labels['home']. '" %s>%s</a>', esc_url( network_home_url() ), $icon, jinx_get_schema_markup('item'), $text );
        }
        ...
        protected function add_site_home_link() {
            ...
            $this->items[] = sprintf( '<a href="%s"%s aria-label="' .$this->labels['home']. '" %s>%s%s</a>', esc_url( home_url() ), $rel, jinx_get_schema_markup('item'), $icon, $label );
        }
        ...
        protected function add_singular_items() {
            ...
            // Post type taxonomy
            $posts_tax 		= jinx_get_option( 'breadcrumb_posts_taxonomy', 'none' );
            $products_tax 	= jinx_get_option( 'breadcrumb_products_taxonomy', 'shop' );
            $portfolio_tax 	= jinx_get_option( 'breadcrumb_portfolio_taxonomy', 'la_portfolio_category' );
            ...
            if ( 'post' === $post->post_type && 'none' != $posts_tax ) {
                ...
                if ( 'blog' == $posts_tax
                    ...
                    $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_permalink( $page_id ) ), jinx_get_schema_markup('item'), get_the_title( $page_id ) );
                }
                else {
            ...
            // If the post type is 'product'.
                ...
                if ( 'shop' == $products_tax ) {
                    ...
                    $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_permalink( $shop_id ) ), jinx_get_schema_markup('item'), get_the_title( $shop_id ) );
                }
                if ( 'all' == $products_tax ) {
                    $shop_id = wc_get_page_id( 'shop' );
                    $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_permalink( $shop_id ) ), jinx_get_schema_markup('item'), get_the_title( $shop_id ) );
            ...
            // If the post type is 'lastudio_portfolio'.
                ...
                $portfolio_id = jinx_get_option( 'portfolio_page' );

                if ( 'portfolio' == $portfolio_tax ) {
                    if( ! empty( $portfolio_id )){
                        $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_permalink( $portfolio_id ) ), jinx_get_schema_markup('item'), get_the_title( $portfolio_id ) );
                    }
                    else{
                        $post_type_object = get_post_type_object( 'la_portfolio' );
                        $label = ! empty( $post_type_object->labels->archive_title ) ? $post_type_object->labels->archive_title : $post_type_object->labels->name;
                        $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_post_type_archive_link( 'la_portfolio' ) ), jinx_get_schema_markup('item'), esc_html($label) );
                    }
                }
                else {
            ...
            if ( $post_title = single_post_title( '', false ) ) {
                ...
                if ( 1 < get_query_var( 'page' ) || is_paged() )
                    $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_permalink( $post_id ) ), jinx_get_schema_markup('item'), $post_title );
                    ...
                    if ( isset( $matches ) ) {
                        ...
                        foreach ( $matches as $match ) {
                            ...
                            if ( !empty( $post_types ) ) {
                                ...
                                $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_post_type_archive_link( $post_type_object->name ) ), jinx_get_schema_markup('item'), $label );
            ...
            if ( false === $done_post_type && 1 === count( $taxonomy->object_type ) && post_type_exists( $taxonomy->object_type[0] ) ) {
                ...
                if ( 'post' === $taxonomy->object_type[0] ) {
                    ...
                    if ( 'posts' !== get_option( 'show_on_front' ) && 0 < $post_id )
                        $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_permalink( $post_id ) ), jinx_get_schema_markup('item'), get_the_title( $post_id ) );
                    ...
                    $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( $url ), jinx_get_schema_markup('item'), $label );
                }
            }
        ...
        protected function add_post_type_archive_items() {
            ...
            if ( is_paged() ){
                $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_post_type_archive_link( $post_type_object->name ) ), jinx_get_schema_markup('item'), post_type_archive_title( '', false ) );
            }
            elseif ( true === $this->args['show_title'] ){
        ...
        protected function add_user_archive_items() {
            ...
            if ( is_paged() )
                $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_author_posts_url( $user_id ) ), jinx_get_schema_markup('item'), get_the_author_meta( 'display_name', $user_id ) );
        ...
        protected function add_minute_hour_archive_items() {
            ...
            if ( true === $this->args['show_title'] )
                $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'jinx' ) ) );
        }
        ...
        protected function add_minute_archive_items() {
            ...
            // Add the minute item.
            if ( true === $this->args['show_title'] )
                $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'jinx' ) ) );
        }
        ...
        protected function add_hour_archive_items() {
            ...
            // Add the hour item.
            if ( true === $this->args['show_title'] )
                $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'jinx' ) ) );
        }
        ...
        protected function add_day_archive_items() {
            ...
            $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'jinx' ) ) );
            $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'jinx' ) ) );
            $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'jinx' ) ) );

            // Add the year and month items.
            $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_year_link( get_the_time( 'Y' ) ) ), jinx_get_schema_markup('item'), $year );
            $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) ), jinx_get_schema_markup('item'), $month );

            // Add the day item.
            if ( is_paged() )
                $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_day_link( get_the_time( 'Y' ) ), get_the_time( 'm' ), get_the_time( 'd' ) ), jinx_get_schema_markup('item'), $day );
        ...
        protected function add_week_archive_items() {
            ...
            $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'jinx' ) ) );
            $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'jinx' ) ) );
        ...
        protected function add_search_items() {
            ...
            if ( is_paged() )
                $this->items[] = sprintf( '<a href="%s" %s>%s%s</a>', esc_url( get_search_link() ), jinx_get_schema_markup('item'), '<span class="breadcrumb-search">'. $this->labels['search'] .'</span>', ' &#8220;'. get_search_query() .'&#8221;' );
        ...
        protected function add_post_parents( $post_id ) {
            ...
            while ( $post_id ) {
                ...
                $parents[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_permalink( $post_id ) ), jinx_get_schema_markup('item'), get_the_title( $post_id ) );
        ...
        protected function add_post_hierarchy( $post_id ) {
            ...
            if ( $post_type_object->has_archive ) {
                ...
                $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_post_type_archive_link( $post_type ) ), jinx_get_schema_markup('item'), $label );
            }
        }
        ...
        protected function add_post_terms( $post_id, $taxonomy ) {
            ...
            if ( $terms && ! is_wp_error( $terms ) ) {
                ...
                $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_term_link( $term, $taxonomy ) ), jinx_get_schema_markup('item'), $term->name );
            }
        }
        ...
        function add_term_parents( $term_id, $taxonomy ) {
            ...
            while ( $term_id ) {
                ...
                $parents[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_term_link( $term, $taxonomy ) ), jinx_get_schema_markup('item'), $term->name );
        ...
        protected function map_rewrite_tags( $post_id, $path ) {
            ...
            if ( is_array( $matches ) ) {
                ...
                foreach ( $matches as $match ) {
                    ...
                    if ( '%year%' == $tag )
                        $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), jinx_get_schema_markup('item'), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'jinx' ) ) ) );

                    // If using the %monthnum% tag, add a link to the monthly archive.
                    elseif ( '%monthnum%' == $tag )
                        $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), jinx_get_schema_markup('item'), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'jinx' ) ) ) );

                    // If using the %day% tag, add a link to the daily archive.
                    elseif ( '%day%' == $tag )
                        $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), jinx_get_schema_markup('item'), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'jinx' ) ) ) );

                    // If using the %author% tag, add a link to the post author archive.
                    elseif ( '%author%' == $tag )
                        $this->items[] = sprintf( '<a href="%s" %s>%s</a>', esc_url( get_author_posts_url( $post->post_author ) ), jinx_get_schema_markup('item'), get_the_author_meta( 'display_name', $post->post_author ) );
