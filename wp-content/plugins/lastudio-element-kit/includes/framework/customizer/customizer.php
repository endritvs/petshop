...
if ( ! class_exists( 'CX_Customizer' ) ) {
	...
	class CX_Customizer {
		...
		public function add_control( $id, $args ) {
			...
			$this->customize->add_setting( $id, $args_settings );
            ...
            if ( isset( $args['responsive'] ) && $args['responsive'] ) {
                $settings_laptop            = $args_settings;
                $settings_laptop['default'] = isset( $settings_laptop['default_laptop'] ) ? $settings_laptop['default_laptop'] : $settings_laptop['default'];
                $this->customize->add_setting( $id . '_laptop', $settings_laptop );

                $settings_tablet            = $args_settings;
                $settings_tablet['default'] = isset( $settings_tablet['default_tablet'] ) ? $settings_tablet['default_tablet'] : $settings_tablet['default'];
                $this->customize->add_setting( $id . '_tablet', $settings_tablet );

                $settings_ml            = $args_settings;
                $settings_ml['default'] = isset( $settings_ml['default_mobile_extra'] ) ? $settings_ml['default_mobile_extra'] : $settings_ml['default'];
                $this->customize->add_setting( $id . '_mobile_extra', $settings_ml );

                $settings_mobile            = $args_settings;
                $settings_mobile['default'] = isset( $settings_mobile['default_mobile'] ) ? $settings_mobile['default_mobile'] : $settings_mobile['default'];
                $this->customize->add_setting( $id . '_mobile', $settings_mobile );

                $control_args['responsive'] = true;
                $control_args['settings'] = array(
                    'default'          => $id,
                    'desktop'          => $id,
                    'laptop'           => $id . '_laptop',
                    'tablet'           => $id . '_tablet',
                    'mobile_extra'     => $id . '_mobile_extra',
                    'mobile'           => $id . '_mobile',
                );
            }
			...
			switch ( $field_type ) {
				...
				case 'image':
						...
						if(isset($args['button_labels'])){
						    $control_args['button_labels'] = $args['button_labels'];
                        }
					break;
				...
				default:
						...
						$control_args = apply_filters(
							'lastudio-kit/theme/customizer/control_args/{$field_type}',
							$control_args,
							$id,
							$this
						);
			 ...
			 * @param object $this         Cherry_Customizer instance.
			 */
			$control_args = apply_filters( 'lastudio-kit/theme/customizer/control_args', $control_args, $id, $this );
		...
		public function get_control_section( $args ) {
			...
			$default_sections = apply_filters( 'lastudio-kit/theme/customizer/core_sections', array(
				'title_tagline',
				'colors',
				'header_image',
				'background_image',
		...
		public function get_image_types() {
			 ...
			 * @param object $this  CX_Customizer instance.
			 */
			return apply_filters( 'lastudio-kit/theme/customizer/image_types', array(
		...
		public function prepare_fonts() {
			 ...
			 */
			$this->fonts = apply_filters( 'lastudio-kit/theme/customizer/fonts_list', $this->fonts, $this );
		...
		public function get_fonts_data() {
			 ...
			 * @param object $this CX_Customizer instance.
			 */
			return apply_filters( 'lastudio-kit/theme/customizer/fonts_data', array(
        ...
        public function custom_devices( $devices ) {
            $laptop           = array(
                'laptop' => array(
                    'label' => __( 'Enter laptop preview mode', 'lastudo-kit' ),
                ),
            );
            $mobile_landscape = array(
                'mobile_extra' => array(
                    'label' => __( 'Enter mobile landscape preview mode', 'lastudo-kit' ),
                ),
            );
