...
class LaStudio_Kit_Nav_Walker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$class_names = $class_names ? ' class="' . esc_attr( $class_names ) . '"' : '';
		$li_style = '';
		if(!empty($settings['custom_mega_menu_width'])){
			$inline_styles['--mm-sub-width'] = $settings['custom_mega_menu_width'] . 'px';
		}
		...
		if(0 === $depth && $show_megamenu && $this->is_mega_enabled( $item->ID )){
			...
			if(!empty($template_content)){
				...
				$item_output .= sprintf('<div class="lakit-nav__sub lakit-nav__sub-mega lakit-nav-depth-%3$s" data-position="%2$s">%1$s</div>', $template_content, $position, $depth);
			}
		}
